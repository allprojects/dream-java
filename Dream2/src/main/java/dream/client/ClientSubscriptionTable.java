...
final class ClientSubscriptionTable {
	...
	final Set<Subscriber> getSignatureOnlyMatchingSubscribers(Event ev) {
		return getSubscribersWithAnySubscriptionMatching(
				sub -> sub.matchesOnlySignatureOf(ev) && !sub.isSatisfiedBy(ev));
	}

	private final Set<Subscriber> getSubscribersWithAnySubscriptionMatching(Predicate<Subscription> predicate) {
		final Predicate<Subscriber> hasSubscriptionMatchingOnlySignature = subscriber -> subs.get(subscriber).stream()
				.anyMatch(predicate);
