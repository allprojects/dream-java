...
public abstract class Proxy implements Subscriber, ProxyGenerator {
	...
	public Proxy(String host, String object) {
		...
		// forwarder = ClientEventForwarder.get();
		// proxyID = UUID.randomUUID();
		// Subscription sub = new Subscription(host, object, proxyID, new
		// Constraint(method));
		// forwarder.addSubscription(this, sub);
	}
	...
	/**
	 * Method invoked by a ProxyChangeListener to acknowledge that the
	 * eventPacket has been processed. After receiving this acknowledgement from
	 * all registered ProxyChangeListeners, the Proxy can safely start
	 * processing the next EventPacket received (if any).
	 * 
