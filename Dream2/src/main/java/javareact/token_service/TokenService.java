...
/**
 * A token service ensures that only one event (called active event) is
 * propagated through the broker network at any given time. In particular, it
 * stores incoming events in a queue and delivers them only when no more
 * forwarding are planned for the currently active event.
 */
