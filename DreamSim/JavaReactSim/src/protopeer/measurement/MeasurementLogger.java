...
public class MeasurementLogger {
		...
		epochEndTimer.addTimerListener(new TimerListener() {
			public void timerExpired(Timer timer) {
				if (timer == epochEndTimer) {
					fireMeasurmentEpochEnded(getCurrentLog(), (int) (Time.inMeasurementEpochs(Time
							.inMilliseconds(getClock().getCurrentTime()))) - 1);
	...
	private double getTimeUntilNextEpoch(double currentTime) {
		// add a 20ms margin to be sure the timer expires in the new epoch
		// FIXME: verify if this is really necessary
		return (Math.floor(currentTime / MainConfiguration.getSingleton().measurementEpochDuration) + 1)
				* MainConfiguration.getSingleton().measurementEpochDuration - currentTime + 20.0;
	}
