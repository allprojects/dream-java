		...
		if (!eventsList.isEmpty()) {
			...
			if (anyPkt.getLockReleaseNodes().contains(object + "@" + host) //
			    && (conf.consistencyType == DreamConfiguration.COMPLETE_GLITCH_FREE
			        && LocalityDetector.instance.sourceRequiresLock(anyPkt.getSource())
			        || //
	...
	private final void processNextEvent() {
		if (!pendingEvents.isEmpty()) {
			...
			if (conf.consistencyType == DreamConfiguration.COMPLETE_GLITCH_FREE_OPTIMIZED && //
				...
				final boolean lockRequired = forwarder.sendReadWriteLockRequest(nextPacket.getSource(), nextPacket.getId(),
				    this);
				if (!lockRequired) {
					sendNextEventPacket();
					processNextEvent();
