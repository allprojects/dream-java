...
public class Var implements LockApplicant {
	...
	public final void modify() {
		final EventPacket ev = new EventPacket(new Event(host, object), UUID.randomUUID(), peer.getClock().getCurrentTime(),
		    object + "@" + host);
	...
	private final void processNextEvent() {
		if (!pendingEvents.isEmpty()) {
			...
			if (conf.consistencyType == DreamConfiguration.COMPLETE_GLITCH_FREE
			    && LocalityDetector.instance.sourceRequiresLock(object + "@" + host)
			    || //
