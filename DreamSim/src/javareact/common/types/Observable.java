...
public abstract class Observable implements ProxyGenerator {
  ...
  private final void sendAdvertisement() {
    Advertisement adv = new Advertisement(observableId, Consts.hostPrefix + ((TrafficGeneratorPeerlet) peer.getPeerletOfType(TrafficGeneratorPeerlet.class)).getClientId());
    forwarder.advertise(adv, true);
  }

  protected synchronized void sendEvent(Attribute[] attributes) {
    Event ev = new Event(observableId, Consts.hostPrefix + ((TrafficGeneratorPeerlet) peer.getPeerletOfType(TrafficGeneratorPeerlet.class)).getClientId(), persistent, attributes);
